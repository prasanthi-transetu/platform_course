...
export default function DeleteQuizStep1Page() {
  ...
  const handleDelete = () => {
    if (isLinked) {
      router.push(`/admin/quizzes/delete/step2/${params.id}`);
    } else {
    ...
    <div className="min-h-screen bg-slate-900/10 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
        ...
        <div className="p-8">
          ...
          <div
            className="bg-slate-50 border border-gray-200 rounded-xl p-4 flex items-center gap-4 cursor-pointer hover:border-blue-400 transition-colors"
            onClick={() => setIsLinked(!isLinked)}
          >
            <div
              className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all flex-shrink-0 ${
                isLinked ? "bg-blue-600 border-blue-600 shadow-md shadow-blue-200" : "border-gray-300"
              }`}
            >
              {isLinked && <div className="w-1.5 h-1.5 bg-white rounded-full" />}
            </div>
